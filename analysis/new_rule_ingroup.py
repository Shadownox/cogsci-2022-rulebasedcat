...
rule_change = df[df["rules_set"] == "ruleset2b"]
...
for idx, data in new_rule_by_task:
    ...
    item_name = item_name.replace("test_memory_p3", old_rule).replace("test_memory_p4", old_rule)
    item_name = item_name.replace("test_learning_p3", new_rule).replace("test_learning_p4", new_rule)
    item_name = item_name.replace("upper_bound_old_p3", consensus_old).replace("upper_bound_old_p4", consensus_old)
    item_name = item_name.replace("upper_bound_new_p3", consensus_new).replace("upper_bound_new_p4", consensus_new)
    item_name = item_name.replace("interference_old_p3", conflict_old).replace("interference_old_p4", conflict_old)
    item_name = item_name.replace("interference_new_p3", conflict_new).replace("interference_new_p4", conflict_new)
...
ax.set_title("New Rule (within division)", fontsize=14)
ax.set_xticklabels(order, fontsize=12.5)
ax.set_xlabel("")
ax.set_ylabel("")
plt.ylim(0, 1)

plt.tight_layout()
plt.savefig("new_rule_ingroup_corr.pdf")
plt.show()
